...
<template>
  <section class="news_section">
    ...
    <div class="title-con">
      <span class="title">{{ rooms[4].title }}</span>
    </div>
    <div class="container">
      <div class="img-con">
      ...
      <div class="info-con">
        <div class="info">
          ...
          <div class="schedule-con">
            ...
            <p class="sub-title">
              인원 초과시 인당
              {{ this.rooms[4].numPrice.toLocaleString() }}원 추가 비용 발생
            </p>
            <v-text-field
              label="인원수를 입력해 주세요."
              persistent-hint
...
export default {
  ...
  methods: {
    ...
    plusBtn() {
      this.num++;
      if (this.num >= this.rooms[4].numMax) {
        this.num = this.rooms[4].numMax;
      }
    ...
    PriceCalc() {
      this.numPrice =
        this.rooms[4].numPrice * (this.num - this.rooms[4].numMin);
      if (this.num <= this.rooms[4].numMin) {
        this.numPrice = 0;
      }
